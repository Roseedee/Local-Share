...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 30, 2026 at 01:45 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_file_permission_list` (IN `file_id` INT)   SELECT
  dpa.allowed_device_id     AS device_id,
  c.client_name,
  #f.owner_device_id         AS file_owner_device_id,
  COALESCE(fp.permissions_code, 'r--') AS permission_code,
  CASE
    WHEN fp.permissions_code IS NOT NULL THEN 'FILE'
    ELSE 'SCOPE'
  END AS permission_source
FROM files f
JOIN devices_protected_access dpa
  ON dpa.owner_device_id = f.owner_device_id
LEFT JOIN files_permissions fp
  ON fp.file_id = f.file_id
 AND fp.device_id = dpa.allowed_device_id
 JOIN clients c on c.client_id = dpa.allowed_device_id
WHERE f.file_id = file_id
  AND f.access_scope = 'PROTECTED'$$
